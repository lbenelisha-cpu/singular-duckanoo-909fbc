...
export default function DashboardApp({ currentUser, userRole = 'viewer', isGuest = false, onSignOut, onRequestAdminLogin }) {
  ...
  const safetyYear = (safetyMonth || to || from || iso(new Date())).slice(0,4)
  ...
  const safetySummary = useMemo(() => {
    ...
    const period=sum(keys), annualCalculated=sum(annualKeys)
    const manualForYear=safetyAnnualManual[year]||{}
    const annual=safetyFields.reduce((o,[k]) => (o[k]=safetyAnnualManualEnabled && manualForYear[k]!==undefined ? Number(manualForYear[k]||0) : annualCalculated[k],o),{})
    const ratios = vals => { const significant=Number(vals.lostDays||0)+Number(vals.medical||0), near=Number(vals.nearMiss||0)+Number(vals.unsafe||0); return { firstAid: significant>0&&Number(vals.firstAid||0)>0 ? Number(vals.firstAid)/significant : null, near: significant>0&&near>0 ? near/significant : null } }
    return {period, annual, annualCalculated, periodRatios:ratios(period), annualRatios:ratios(annual), months:keys, year, annualManualEnabled:safetyAnnualManualEnabled}
  }, [safetyMonthly, safetyAnnualManual, safetyAnnualManualEnabled, from, to])
