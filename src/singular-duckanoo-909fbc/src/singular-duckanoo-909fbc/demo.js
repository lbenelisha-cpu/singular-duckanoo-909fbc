import { matchResourceMapping } from './resourceMappings'
...
const matchProductionToTarget = (row, target, mappings) => {
  ...
  const mapped = matchResourceMapping(row, mappings)
  if (mapped) return upper(mapped.resource) === upper(target.resource)
  // Facility 42 must never fall back to a partial token match. An unknown
  // Description is intentionally excluded and reported by Mapping Monitor.
  if (row.facility === '1542') return false
...
export function buildResourceRows({ production = [], targets = [], planningMonth = '', fallbackFacilities = [], mappings = [], now = new Date() }) {
  ...
  return sourceRows.map((targetRow, index) => {
    const rows = monthRows.filter(row => matchProductionToTarget(row, targetRow, mappings))
