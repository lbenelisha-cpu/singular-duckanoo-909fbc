...
import {
  ...
  Gauge, CalendarCheck, BellRing, TrendingUp, FileSpreadsheet, ShieldCheck, RefreshCw, ClipboardList, Activity, LogOut, UserCircle, Cloud, WifiOff, ArrowLeft, HeartPulse, Settings2, Plus, MapPin
} from 'lucide-react'
...
import { DEFAULT_RESOURCE_MAPPINGS, summarizeUnmappedRows, loadResourceMappingsFromCloud, saveResourceMappingsToCloud } from './resourceMappings'
import './styles.css'
...
export default function DashboardApp({ currentUser, userRole = 'viewer', isGuest = false, onSignOut }) {
  ...
  const [resourceMappings, setResourceMappings] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('iml-resource-mappings-v1') || 'null')
      return Array.isArray(saved) && saved.length ? saved : DEFAULT_RESOURCE_MAPPINGS
    } catch { return DEFAULT_RESOURCE_MAPPINGS }
  })
  ...
  setProduction(
    ...
    ;(async () => {
      ...
      try {
        cached = await idbGet()
        if (active && cached) {
          console.log('Cached production rows:', (cached.production || []).length)

console.table(
  (cached.production || [])
    .filter(r => String(r.facility) === '1542')
    .map(r => ({
      batch: r.batch,
      qty: r.qty,
      routingGroup: r.routingGroup,
      order: r.order,
      date: r.finishDate || r.date,
    }))
)
  ...
  useEffect(() => {
    localStorage.setItem('iml-resource-mappings-v1', JSON.stringify(resourceMappings))
  }, [resourceMappings])

  useEffect(() => {
    let active = true
    loadResourceMappingsFromCloud(supabase).then(rows => {
      if (active && Array.isArray(rows) && rows.length) setResourceMappings(rows)
    }).catch(error => console.warn('Resource mappings cloud load failed', error))
    return () => { active = false }
  }, [])

  const saveResourceMappings = async rows => {
    setResourceMappings(rows)
    try {
      await saveResourceMappingsToCloud(supabase, rows)
      setStatus(`מיפוי המשאבים נשמר בענן — ${rows.length} כללים`)
    } catch (error) {
      console.error(error)
      setStatus(`המיפוי נשמר מקומית, אך השמירה בענן נכשלה: ${error?.message || 'שגיאה לא ידועה'}`)
    }
  }
  ...
  return <div className="dashboard" dir="rtl">
    ...
    <main className="main">
      ...
      <section className="data-center">
        <div className="panel-head"><div><ShieldCheck/><h2>מרכז נתונים</h2></div><div className="mapping-center-actions"><button type="button" onClick={() => setMappingOpen(true)}><Settings2 size={16}/> מיפוי משאבים</button><span className={unmappedProduction.length ? "mapping-warning" : "mapping-ok"}>{unmappedProduction.length ? `${unmappedProduction.length} תיאורים לא ממופים` : "כל התיאורים ממופים"}</span></div></div>
      ...
      <section className={`mapping-monitor ${unmappedProduction.length ? 'has-unmapped' : 'all-mapped'}`}>
        <div><MapPin/><div><h2>Mapping Monitor</h2><p>רשומות ללא מיפוי אינן נכנסות לחישובי היעדים והתחזיות.</p></div></div>
        <div className="mapping-monitor-kpis"><span>תיאורים לא ממופים <b>{unmappedProduction.length}</b></span><span>רשומות <b>{fmt(unmappedProduction.reduce((sum,row)=>sum+row.rows,0))}</b></span><span>כמות לא ממופה <b>{fmt(unmappedQty)}</b></span></div>
        {unmappedProduction.length ? <button type="button" onClick={() => setMappingOpen(true)}>פתח וטפל במיפויים</button> : <strong>✓ כל רשומות הייצור משויכות למשאב מוגדר</strong>}
      </section>
    ...
    {mappingOpen && <ResourceMappingModal mappings={resourceMappings} unmapped={unmappedProduction} onSave={saveResourceMappings} onClose={() => setMappingOpen(false)}/>}
...
function ResourceMappingModal({ mappings, unmapped, onSave, onClose }) {
  const [draft, setDraft] = useState(() => mappings.map(item => ({...item})))
  useEffect(() => {
  ...
  const update = (id, key, value) => setDraft(rows => rows.map(row => row.id === id ? {...row,[key]:value} : row))
  const add = (seed={}) => setDraft(rows => [...rows, { id:`map-${Date.now()}-${rows.length}`, facility:seed.facility||'', description:seed.description||'', mode:'exact', resource:'', active:true }])
  const remove = id => setDraft(rows => rows.filter(row => row.id !== id))
  const save = () => { onSave(draft.filter(row => row.facility && row.description && row.resource)); onClose() }
  const reset = () => setDraft(DEFAULT_RESOURCE_MAPPINGS.map(item => ({...item})))
  return <div className="resource-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <section className="mapping-modal" role="dialog" aria-modal="true">
      <header><div><span>RESOURCE MAPPING ENGINE</span><h2>מיפוי תחנה + Description למשאב</h2><p>במתקן 42 החישוב מתבצע רק לפי התאמה מדויקת. תיאור לא מוכר לא ייספר בקו אחר.</p></div><button onClick={onClose}><X/></button></header>
      {!!unmapped.length && <section className="unmapped-box"><div><AlertTriangle/><strong>נמצאו {unmapped.length} תיאורים לא ממופים</strong></div><div className="unmapped-chips">{unmapped.slice(0,20).map(row => <button key={`${row.facility}-${row.description}`} onClick={() => add(row)}><b>{row.facility}</b><span>{row.description}</span><small>{fmt(row.qty)} · {row.rows} רשומות</small><Plus size={15}/></button>)}</div></section>}
